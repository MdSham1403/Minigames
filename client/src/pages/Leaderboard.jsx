...
const GAMES = [
  ...
  { id:'trivia',      label:'❓ Trivia',                desc:'Best quiz score' },
  ...
  { id:'reaction',    label:'⚡ Reaction Time',         desc:'Fastest average reaction' },
  { id:'numberpuzzle',label:'🔷 15 Puzzle',             desc:'Fewest moves, fastest time' },
  { id:'connect4',    label:'🔴 Connect Four',          desc:'Win score vs AI' },
  { id:'chess',       label:'♟ Chess',                 desc:'Win score vs AI' },
  { id:'ludo',        label:'🎲 Ludo',                  desc:'First to finish all pieces' },
  { id:'uno',         label:'🃏 UNO',                   desc:'Win score vs AI' },
...
const MEDALS = ['🥇','🥈','🥉'];
const RANK_BG = [
  'bg-yellow-500/10 border-l-yellow-400',
  'bg-gray-500/10 border-l-gray-400',
  'bg-orange-600/10 border-l-orange-400',
];
...
const Leaderboard = () => {
  ...
  // Display name — respect privacy
  const displayName = (row) => row.username || 'Anonymous';
  const isAnon      = (row) => row.leaderboard_visible === false || row.username === 'Anonymous';
  ...
  return (
    <div className="min-h-screen bg-gray-950 px-4 py-10">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Privacy note */}
        <div className="flex items-center gap-2 bg-gray-900 border border-gray-800 rounded-xl px-4 py-2.5 mb-5 text-xs text-gray-400">
          <span>🔒</span>
          <span>Players can hide their username in <strong className="text-gray-300">Profile → Leaderboard privacy</strong>. Hidden players show as Anonymous.</span>
        </div>
              ...
              {scores.length >= 3 && (
                <div className="flex items-end justify-center gap-3 bg-gray-900 px-6 pt-8 pb-4 border-b border-gray-800">
                  {[1, 0, 2].map(rank => {
                    ...
                    const podiumH = [56, 80, 40][rank === 0 ? 1 : rank === 1 ? 0 : 2];
                    const anon = isAnon(row);
                    return (
                      <div key={rank} className="flex flex-col items-center gap-2">
                        <div
                          className={`rounded-full flex items-center justify-center font-bold text-white ${rank === 0 ? 'w-14 h-14 ring-2 ring-yellow-400' : 'w-12 h-12'}`}
                          style={{ backgroundColor: anon ? '#4b5563' : (row?.avatar_color || '#6366f1') }}>
                          {anon ? '?' : row?.username?.[0]?.toUpperCase()}
                        </div>
                        <p className="text-xs text-gray-300 font-medium max-w-[70px] truncate text-center">
                          {displayName(row)}
                        </p>
                        ...
                        <div className="rounded-t-xl flex items-end justify-center pb-2 text-2xl"
                          style={{ width: rank === 0 ? 80 : 64, height: podiumH, background: rank === 0 ? 'rgba(234,179,8,0.2)' : rank === 1 ? 'rgba(156,163,175,0.2)' : 'rgba(194,119,73,0.2)' }}>
                          {MEDALS[rank]}
                        </div>
                      </div>
                    );
                  ...
                  return (
                    <div key={i}
                      ...
                      <div className="flex-1 flex items-center gap-2 min-w-0">
                        <span className={`font-medium truncate ${anon ? 'text-gray-500 italic' : 'text-white'}`}>
                          {displayName(row)}
                        </span>
                        {anon && (
        ...
        <p className="text-center text-gray-700 text-xs mt-6">Scores update after every game · Privacy settings respected</p>
      </div>
    </div>
  );
};
